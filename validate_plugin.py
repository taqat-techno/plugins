...
class PluginValidator:
    ...
    def check_commands(self) -> bool:
        ...
        print("   ✓ Command validation complete")
        return True
    ...
    def check_agents(self) -> bool:
        ...
        agents_dirs = list(self.plugin_path.rglob("agents"))
        if not agents_dirs:
    ...
    def _validate_agent_file(self, file_path: Path):
        ...
        # Check for frontmatter
        if not content.startswith("---"):
            self.errors.append(f"Agent {file_path.name} missing YAML frontmatter")
            return
        ...
        for field in required_fields:
            if field not in frontmatter:
                self.warnings.append(f"Agent {file_path.name} missing '{field}' in frontmatter")

        print(f"   ✓ Agent {file_path.name} is valid")
    ...
    def _validate_skill_file(self, file_path: Path):
        ...
        optional_fields = ["allowed-tools", "metadata", "version"]
        for field in optional_fields:
            if field not in frontmatter:
                self.suggestions.append(f"Consider adding '{field}' to skill {file_path.name}")

        print(f"   ✓ Skill {file_path.name} is valid")
    ...
    def check_hooks(self) -> bool:
        ...
        if not hooks_files:
            print("   ℹ️  No hooks.json files found")
        ...
        print("   ✓ Hooks validation complete")
        return True
    ...
    def _validate_hooks_file(self, file_path: Path):
        ...
        print(f"   ✓ Hooks file {file_path.name} is valid")
