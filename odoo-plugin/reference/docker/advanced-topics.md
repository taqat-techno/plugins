...
The full GitHub Actions workflow template is generated by `/docker build --generate-ci`.

### Enterprise projects: Actions cannot build or test them

An ephemeral-Odoo CI job that checks out `odoo/odoo` **and** `odoo/enterprise` only works
for Community. The Enterprise source is a private repository that a GitHub Actions runner
has no token for, so every Enterprise build/install/test job fails at checkout — and it
fails for an access reason no workflow change can fix.

For an Enterprise project hosted on Odoo.sh, split the pipeline by what each side can
actually do:

| Stage | Where it runs | Why |
|---|---|---|
| Static + security gates (SAST/SCA, secret scan, dependency audit, manifest / XML / Python lint) | GitHub Actions | Needs only the module source, which the runner has |
| Build, install, module tests | Odoo.sh | The only place with Enterprise source |
| DAST / dynamic probes | Manual, against the live staging URL | Needs a running instance |

Two wiring traps on this shape:

- **The Git root is often the module folder itself.** These repos put modules at the
  repository root rather than under a project directory, so `.github/` and every CI path is
  relative to that root. Confirm before placing anything:
  `git rev-parse --show-toplevel`.
- **Actions branch filters are case-sensitive.** A deploy branch named `Production` never
  matches a filter written as `production`, and the workflow simply never triggers — no
  error, no run. Pin filters to the exact names from `git branch -a`.
...
Assign unique ports per project in `.env`, offsetting by 100 per project:
```env
PROJECT_A_HTTP_PORT=8069
PROJECT_B_HTTP_PORT=8169
PROJECT_C_HTTP_PORT=8269
```
