...
- `fires-on-optional-tenant-anchor-field` targets `multi-tenancy-isolation`
- `fires-on-public-widget-rpc-unavailable` targets `upgrade`
- `fires-on-standalone-owl-kiosk-screen` targets `owl-architecture`
- `ignores-multi-tenant-saas-architecture` asserts no skill of this plugin fires (`min: 0`, `max: 0`, `arm: both`)
