...
def inject_reminder(
    ...
    reminder = (
        ...
        f"Before answering, search the knowledge base:\n"
        f"  mcp__plugin_rag_ragtools__search_knowledge_base(query=<...>)\n\n"
        f"Treat the knowledge base as project memory/reference, not the sole "
        f"source of truth. Don't say \"I don't have information about X\" for an "
        f"internal question (process / decision / convention / requirement) "
        f"without searching first — but a KB hit is a point-in-time snapshot, so "
        f"route by source of truth: verify current vendor / API / pricing / "
        f"security facts against official docs or the web, inspect the live "
        f"code / runtime for how the code actually behaves, and if the KB "
        f"conflicts with the code or docs, surface the conflict and prefer the "
        f"owning source. Report each load-bearing claim's source "
        f"([from KB] / [from code] / [from official docs] / [assumption]).\n\n"
        f"See ~/.claude/CLAUDE.md Section 0 for the full source-of-truth routing rule."
    )
    output = {
        "hookSpecificOutput": {
