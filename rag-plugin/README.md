...
**Status:** `v0.18.0` — All 10 phases shipped + post-roadmap amendments through **D-037**. 9 commands (8 user-facing + 1 maintainer-only), **4 skills**, 2 hooks, 6 rules, 5 deterministic helper scripts, 261 tests.

**Compatibility band:** supported **ragtools 2.5.0 – 3.5.x**, target **3.5.1**, degraded 2.5.0–2.6.x, unsupported < 2.5.0. The band's single home is `skills/ragtools-ops/references/_meta.md` (D-011) — this line points there rather than restating it, because three different numbers in three files is how it drifted last time.

- **New in v0.18.0 (D-033…D-037):** the retrieval-reminder hook had been **completely non-functional since 2026-07-29** — its relevance probe called `/api/search` with no `project`, which ragtools has refused with HTTP 422 since v3.0.0, so it silent-passed on all 105 subsequent probes and injected nothing. The same unscoped call was taught by the injected `CLAUDE.md` block and implied by the project-focus notice: three layers teaching a call that cannot succeed. This release makes scope mandatory everywhere, merges the two `UserPromptSubmit` hooks into one **scoped** context injector, adds a `ragtools-retrieval` skill (guidance, never invocation — D-034), resolves the service by evidence instead of a hardcoded port (D-036), repairs and verifies citation paths, replaces the permanently-closed `KNOWN_SAFE_FLOOR` gate with capability probes (D-033), and documents all **30** MCP tools instead of 21 behind a drift gate (D-035). **RFC-001 is closed**: enforcement did arrive from the application, as a refusal rather than a default.
